...
/* Omit closing PHP tag to avoid "Headers2 already sent" issues. */
/*function woocommerce_variable_add_to_cart22() {
		global $product, $post;
...
$mes=date("m");
$ano=date("Y");
// Separas fecha y hora del formato almacenado en la tabla

list($fecha, $hora) = explode(' ', $fecha_nacimiento);

// Separas año, mes y dia
list($anio, $mes_nac, $dia_nac) = explode('-', $fecha);

$years = abs($ano -  $anio);
$diff = $mes - $mes_nac;
if ($diff < 0){
	$years--;
} else if (($diff == 0) && ($dia - $dia_nac) < 0){
	$years--;
}
return $years;
}
function get_edad($edad, $tipo){
	if ($tipo == 'inic') return substr($edad, 0, 2);
	else return substr($edad, -2, 2);
}

function get_not_empty($filtros){
	foreach ($filtros as $filtro => $value) {
		if (!(empty($value)) && !($filtro == "edad_inic") &&  !($filtro == "edad_fin")) {return '"%'.$value.'%"';}
	}
}

function get_query($filtros){
	$string =  "";
		foreach ($filtros as $filtro => $value) {
		if (!(empty($value)) && !($filtro == "edad_inic") &&  !($filtro == "edad_fin")) {$string .= ' AND form_value LIKE "%' . $value. '%"';}
	}
	return $string;
}
